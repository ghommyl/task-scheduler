...
const showDescription = defineModel('showDescription', { type: Boolean, required: true });
const showEndDate = defineModel('showEndDate', { type: Boolean, required: true });
...
const showDescriptionEdit = ref(showDescription.value);
const showEndDateEdit = ref(showEndDate.value);
...
function cancelEdit() {
  ...
  showDescriptionEdit.value = showDescription.value;
  showEndDateEdit.value = showEndDate.value;
}
...
function focusDescription() {
  const checkbox = document.getElementById('checkboxDescription');
...
<template>
  <div :class="{ 'bg-gray-200': editMode, 'bg-gray-100': !editMode }"
    class="ml-6 mr-6 mb-3 mt-3 p-4 rounded-3xl flex justify-between items-center flex-row border border-gray-300 transition">
    <div v-if="editMode" class="grow mr-6">
      <input type="text" id="title"
        class="bg-inherit font-sans text-2xl text-gray-700 w-full border border-gray-400 rounded-lg pl-1 mb-1"
        v-model="titleEdit">
      <div class="flex flex-row gap-2 items-center">
        <input type="checkbox" id="checkboxDescription" v-model="showDescriptionEdit" @change="focusDescription">
        <textarea id="description" :disabled="!showDescriptionEdit"
          class="bg-inherit font-sans font-light text-sm text-gray-700 w-full resize-none overflow-hidden disabled:opacity-50 border border-gray-400 pl-1 rounded-md"
          rows="1" @input="autoResize" v-model="descriptionEdit" placeholder="Write your description here"></textarea>
      </div>
      <div class="flex flex-row gap-2 items-center">
        <input type="checkbox" id="checkboxEndDate" v-model="showEndDateEdit">
        <div :class="{ 'opacity-50 pointer-events-none text-gray-900': !showEndDateEdit }"
    ...
    <div v-else class="grow w-0 flex flex-col">
      <span class="bg-inherit font-sans font-semibold text-2xl text-gray-700 overflow-auto text-clip whitespace-pre">{{
        title }}</span>
      <span v-if="showDescription"
        class="bg-inherit font-sans font-light text-sm text-gray-700 break-words whitespace-pre-wrap w-full">{{
          description }}</span>
      <div v-if="showEndDate" class="font-sans font-normal text-base text-gray-700">Ends at {{ formatDate(endDate) }}
      </div>
    </div>
    <div v-if="!editMode" class="flex flex-row-reverse gap-3 items-center">
      <svg @click="emit('delete', props.id)"
        class="align-middle size-6 fill-gray-300 md:hover:fill-gray-400 transition cursor-pointer" version="1.1"
        xmlns="http://www.w3.org/2000/svg" viewBox="0 0 482.428 482.429">
        <title>Delete</title>
        <path
        ...
        <path
          d="M171.144,422.863c7.218,0,13.069-5.853,13.069-13.068V262.641c0-7.216-5.852-13.07-13.069-13.07c-7.217,0-13.069,5.854-13.069,13.07v147.154C158.074,417.012,163.926,422.863,171.144,422.863z" />
        <path
          d="M241.214,422.863c7.218,0,13.07-5.853,13.07-13.068V262.641c0-7.216-5.854-13.07-13.07-13.07c-7.217,0-13.069,5.854-13.069,13.07v147.154C228.145,417.012,233.996,422.863,241.214,422.863z" />
        <path
    ...
    <div v-else class="flex flex-row-reverse gap-2 items-center">
      <svg @click="completeEdit"
        class="size-8 align-middle fill-gray-300 md:hover:fill-gray-400 transition cursor-pointer"
      ...
      <svg @click="cancelEdit" viewBox="0 0 100 100"
        class="stroke-gray-300 md:hover:stroke-gray-400 transition size-6 align-middle cursor-pointer"
